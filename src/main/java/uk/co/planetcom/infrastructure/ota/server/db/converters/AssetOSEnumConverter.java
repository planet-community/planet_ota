...
@Converter
public final class AssetOSEnumConverter implements AttributeConverter<AssetOSEnum, String> {
    @Override
    public String convertToDatabaseColumn(final AssetOSEnum AssetOSEnum) {
        if (AssetOSEnum == null)
            throw new IllegalArgumentException("Cannot convert to DB column; `AssetOS` argument is null.");
    ...
    public AssetOSEnum convertToEntityAttribute(final String s) {
        if (s == null || s.isEmpty())
            throw new IllegalArgumentException("Cannot convert to Entity; String argument (`AssetOS`) is null.");
